...
export default async function handler(
  ...
  try {
    ...
    // Extract unique sets - with separate_editions=true, set info is at top level
    ...
    // Process cards - with separate_editions=true, each card in data[] IS already a single edition
    updateProgress({ message: "Processing card data..." });
    const cardsToInsert: any[] = [];
    
    allCardsData.forEach((card: any) => {
      // With separate_editions=true, card data is at top level
      const setCode = card.set?.id;
      ...
      const imageUrl = card.image ? `https://api.gatcg.com${card.image}` : null;
      const effect = card.effect_raw || card.effect || null;
      ...
      cardsToInsert.push({
        ...
        card_number: card.collector_number || "UNKNOWN",
        element: card.element || null,
        card_type: typeString || "Unknown",
        class: Array.isArray(card.classes) && card.classes.length > 0 
          ? card.classes.join(", ") 
          : null,
        rarity: typeof card.rarity === 'number' 
          ? mapRarityNumber(card.rarity)
          : "UNKNOWN",
